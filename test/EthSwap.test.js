...
contract('EthSwap', ([deployer, investor]) => {
	...
	before(async ()=>{
		...
		//Transfer all token to Ethswap( million)
	...
	describe('buyTokens()', async() => {
		...
		it('Allows user to instantly purchase token from ethSwap for a fixed price', async () =>{
			...
			//Check token balance in EthSwap after purchase
			let ethSwapBalance
			ethSwapBalance = await token.balanceOf(ethSwap.address)
			assert.equal(ethSwapBalance.toString(), tokens('999900'))

			//Check Ethereum balance of EthSwap after purchase
			ethSwapBalance = await web3.eth.getBalance(ethSwap.address)
			assert.equal(ethSwapBalance.toString(), web3.utils.toWei('1', 'ether'))

			//Check logs to ensure events was emitted with the correct data
